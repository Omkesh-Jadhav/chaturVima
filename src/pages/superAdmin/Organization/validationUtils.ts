...
export const validateOrganizationTab = (organizationInfo: OrganizationInfo): ValidationResult[] => {
  const validations: ValidationResult[] = [];
  
  // Check mandatory organization fields
  const mandatoryOrgFields = ['name', 'type', 'size', 'industry', 'email', 'phone', 'city', 'state', 'country'];
  const missingOrgFields = mandatoryOrgFields.filter(field => !organizationInfo[field as keyof OrganizationInfo]?.trim());
...
export const validateDepartmentsTab = (departments: Department[]): ValidationResult[] => {
  const validations: ValidationResult[] = [];
  ...
  const departmentNames = departments.map(dept => dept.name.toLowerCase().trim()).filter(name => name);
  const duplicateNames = departmentNames.filter((name, index) => departmentNames.indexOf(name) !== index);
...
export const validateEmployeesTab = (employees: Employee[]): ValidationResult[] => {
  const validations: ValidationResult[] = [];
  ...
  const emails = employees.map(emp => emp.email.toLowerCase().trim()).filter(email => email);
  ...
  const employeeIds = employees.map(emp => emp.employeeId.trim()).filter(id => id);
...
export const validateNoCircularReporting = (employees: Employee[]): ValidationResult => {
  ...
  for (const employee of employees) {
    ...
    while (current) {
      if (visited.has(current)) {
        ...
        const names = cycle.map(id => employeeMap.get(id)?.name || id).join(' → ');
        return {
          isValid: false,
...
export const validateOverallSetup = (
  ...
  const allValidations = [...orgValidations, ...deptValidations, ...empValidations];
