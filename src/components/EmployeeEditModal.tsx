...
const EmployeeEditModal: React.FC<EmployeeEditModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/30 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={handleClose}>
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-gray-200 bg-linear-to-r from-gray-50 to-white">
          ...
          <Button onClick={handleClose} variant="ghost" size="sm" className="text-gray-400 hover:text-gray-600 hover:bg-gray-100">
            <X className="w-5 h-5" />
          </Button>
        </div>
        ...
        <div className="p-6 overflow-y-auto flex-1">
          {isLoadingDetails && (
            <div className="flex items-center justify-center py-12">
              ...
              <span className="text-sm text-gray-600">Loading employee details...</span>
            </div>
          )}

          {(detailsError || editEmployeeMutation.error) && (
            <div className="mb-4 bg-red-50 border border-red-200 rounded-lg p-3">
              <div className="text-xs text-red-800">
                {detailsError ? "Failed to load employee details. Using basic information." : "Failed to save employee details. Please try again."}
              </div>
            </div>
          )}
